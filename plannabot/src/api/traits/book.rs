...
impl fmt::Display for BookParams {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            BookParams::R3(t, s2, od, ot, nd, nt)       => format_screen_spaces!(r, t, s2, od, ot, nd, nt),
            BookParams::RF(t, s2, od, ot, nd, nt)       => format_screen_spaces!(rf, t, s2, od, ot, nd, nt),
            BookParams::S0(t, s2, date, time)           => format_screen_spaces!(s, t, s2, date, time),
            BookParams::SF(t, s2, date, time, status)   => format_screen_spaces!(sf, t, s2, date, time, status),
        }.fmt(f)
    }
}
...
impl From<&BookParams> for MarkdownString {
    fn from(p: &BookParams) -> MarkdownString {
        ...
        match p {
            BookParams::M0 | BookParams::C0() | BookParams::L0(_) | BookParams::U0(_, _) | BookParams::U1(_) => {}
